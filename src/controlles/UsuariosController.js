...
class UsuariosController {
  ...
  async adicionar(req, resp) {
    try {
      ...
      if (!novoUsuario.nome || !novoUsuario.email || !novoUsuario.senha || isNaN(Number(novoUsuario.cpf)) || !novoUsuario.dtNascimento || !novoUsuario.estado || !novoUsuario.cidade || !novoUsuario.bairro || !novoUsuario.rua || isNaN(Number(novoUsuario.numero))) {
        resp.status(400).send('Os campos nome, e-mail, senha, CPF, data de nascimento, estado, cidade, bairro, rua e numero são obrigatórios.');
        return;
      }
  ...
  async atualizar(req, resp) {
    try {
      ...
      if (!usuarioEditar.nome || !usuarioEditar.email || !usuarioEditar.senha || !usuarioEditar.estado || !usuarioEditar.cidade || !usuarioEditar.bairro || !usuarioEditar.rua || isNaN(Number(usuarioEditar.numero))) {
